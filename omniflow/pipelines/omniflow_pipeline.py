...
class OmniFlowPipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin):
    ...
    @staticmethod
    def load_pretrained(path, device='cuda', weight_dtype=torch.bfloat16, load_ema=False, verbose=False):
        """
        加载预训练模型
        ...
        Args:
            ...
            weight_dtype: 权重数据类型
            load_ema: 是否加载 EMA 权重
            verbose: 是否显示加载进度 (默认 True)
        """
        ...
        if verbose:
            print("\n" + "="*70)
            print("🚀 开始加载 OmniFlow Pipeline".center(70))
            print("="*70 + "\n")
        
        # ============ 1. Tokenizers ============
        log("步骤 1/12: 加载 Tokenizers", "📝")
        
        tokenizer_one = CLIPTokenizer.from_pretrained(
            'laion/CLIP-ViT-L-14-DataComp.XL-s13B-b90K',
        )
        log_success("CLIP Tokenizer L")
        
        tokenizer_two = CLIPTokenizer.from_pretrained(
            path,
            subfolder="tokenizer_2",
        )
        log_success("CLIP Tokenizer G")
        
        tokenizer_three = T5TokenizerFast.from_pretrained(
            'google/flan-t5-large',
        )
        log_success("T5 Tokenizer")
        
        # ============ 2. Text Encoders ============
        log("\n步骤 2/12: 加载 Text Encoders", "🔤")
        
        text_encoder_one = CLIPTextModelWithProjection.from_pretrained(
            'laion/CLIP-ViT-L-14-DataComp.XL-s13B-b90K',
            projection_dim=768
        )
        log_success("CLIP Text Encoder L (123M params)")
        
        text_encoder_two = CLIPTextModelWithProjection.from_pretrained(
            path,
            subfolder="text_encoder_2",
        )
        log_success("CLIP Text Encoder G (354M params)")
        
        text_encoder_three = T5EncoderModel.from_pretrained('google/flan-t5-large')
        log_success("T5 Text Encoder (738M params)")
        ...
        # ============ 3. VAE ============
        log("\n步骤 3/12: 加载 Image VAE", "🎨")
        
        vae = AutoencoderKL.from_pretrained(
            path,
            subfolder="vae",
        )
        log_success("Image VAE (Encoder + Decoder)")
        ...
        log_success("Text VAE Tokenizer")
        ...
        log_success("Text VAE (LLaMA-based Latent Connector)")
        
        # ============ 5. Image Encoder ============
        log("\n步骤 5/12: 加载 Image Encoder", "🖼️")
        
        image_encoder = CLIPVisionModelWithProjection.from_pretrained(
            'laion/CLIP-ViT-L-14-DataComp.XL-s13B-b90K',
            projection_dim=768
        )
        log_success("CLIP Vision Encoder (ViT-L/14)")
        
        image_processor = CLIPImageProcessor.from_pretrained(
            'laion/CLIP-ViT-L-14-DataComp.XL-s13B-b90K'
        )
        log_success("CLIP Image Processor")
        
        # ============ 6. Transformer ============
        log("\n步骤 6/12: 加载 OmniFlow Transformer", "🤖")
        ...
        log_success("OmniFlow Transformer (18 layers)")
        
        # ============ 7. Audio Encoder ============
        log("\n步骤 7/12: 加载 Audio Encoder", "🎵")
        
        audio_encoder = LanguageBindAudio.from_pretrained(
        ...
        log_success("LanguageBind Audio Encoder")
        ...
        log_success("所有参数已冻结 (推理模式)")
        
        # ============ 9. 加载权重 ============
        log("\n步骤 9/12: 加载 Transformer 权重", "💾")
        ...
        log_success(f"已加载权重: {fp}")
        
        # ============ 10. 移动到设备 ============
        log(f"\n步骤 10/12: 移动模型到 {device}", "🚀")
        ...
        vae.to(device)
        log_success(f"所有模型已移动到 {device}")
        
        # ============ 11. Audio VAE ============
        log("\n步骤 11/12: 加载 Audio VAE", "🎼")
        if verbose:
            print("   ⏳ 正在下载/加载 Audio VAE (可能需要几分钟)...")
        
        audiovae, audio_processor = load_audio_vae()
        audiovae.to(device)
        audiovae.requires_grad_(False)
        log_success("Audio VAE 加载完成")
        
        # ============ 12. Scheduler ============
        log("\n步骤 12/12: 加载 Scheduler 并组装 Pipeline", "⚙️")
        
        noise_scheduler = OmniFlowMatchEulerDiscreteScheduler.from_pretrained(
            path, subfolder="scheduler", shift=3
        )
        log_success("Flow Scheduler")
        ...
        if load_ema:
            ...
            log_success("EMA 权重已应用")
        
        # ============ 完成 ============
        if verbose:
            print("\n" + "="*70)
            print("✅ Pipeline 加载完成!".center(70))
            print("="*70 + "\n")
            
            # 显示组件总结
            print("📦 加载的组件:")
            print(f"   • Text Encoders: CLIP-L, CLIP-G, T5-Large")
            print(f"   • Image: VAE + CLIP Vision")
            print(f"   • Audio: LanguageBind + Audio VAE")
            print(f"   • Transformer: OmniFlow (18 layers)")
            print(f"   • Text VAE: LLaMA-based")
