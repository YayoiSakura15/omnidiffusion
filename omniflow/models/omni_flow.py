...
from omniflow.models.text_decoder_head import TextDecoderHead, TextDecoderHeadWithCodebook
...
class OmniFlowTransformerModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        use_text_decoder_head: bool = False,
        text_decoder_head_type: str = 'basic',
        text_decoder_head_dim: int = 2048,
        text_decoder_head_hidden_dim: int = 4096,
        use_vq_codebook: bool = False,
        ...
        # 添加 Text Decoder Head 模块
        self.use_text_decoder_head = use_text_decoder_head
        if use_text_decoder_head:
            if use_vq_codebook:
                self.text_decoder_head = TextDecoderHeadWithCodebook(
                    input_dim=self.text_out_dim,
                    hidden_dim=text_decoder_head_hidden_dim,
                    output_dim=text_decoder_head_dim,
                    num_embeddings=vq_num_embeddings,
                )
            else:
                self.text_decoder_head = TextDecoderHead(
                    input_dim=self.text_out_dim,
                    hidden_dim=text_decoder_head_hidden_dim,
                    output_dim=text_decoder_head_dim,
                )
            # 更新 text_out_dim 为上采样后的维度
            self.final_text_dim = text_decoder_head_dim
        else:
            self.text_decoder_head = None
            self.final_text_dim = self.text_out_dim
    ...
    def forward(
        ...
        if do_text and use_text_output:
            ...
            # 应用 Text Decoder Head（如果启用）
            if self.use_text_decoder_head and self.text_decoder_head is not None:
                if hasattr(self.text_decoder_head, 'embedding'):  # VQ variant
                    encoder_hidden_states, vq_loss = self.text_decoder_head(encoder_hidden_states, return_loss=True)
                else:
                    encoder_hidden_states = self.text_decoder_head(encoder_hidden_states)
